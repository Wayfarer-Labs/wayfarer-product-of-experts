...
from experts import GenerativeExpert_Axis, DiscriminativeExpert_Color, GenerativeExpert_Color
...
expert_rgb = GenerativeExpert_Color(color='rgb', device=DEVICE, n_epochs=N_EPOCHS)
...
denoised_trajectory_poe: Tensor = sample_product_of_experts(
    generative_experts=[expert_x, expert_y, expert_z, expert_rgb],
...
denoised_trajectory_noc = sample_product_of_experts(
...
denoised_trajectory_nod = sample_product_of_experts(
    generative_experts=[expert_x, expert_y, expert_z, expert_rgb],
    discriminative_experts=[],
    n_points=N_POINTS,
    n_denoise_steps=128,
    n_seek_steps=3,
    step_size=1.0,
...
vis.render_points_over_time(denoised_trajectory_noc, path=VISUALIZATION_DIR / "experts_noc.gif", show_path=True)
vis.render_points_over_time(denoised_trajectory_nod, path=VISUALIZATION_DIR / "experts_nod.gif", show_path=True)